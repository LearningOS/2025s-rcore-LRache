...
use crate::{
    config::PAGE_SIZE, fs::{open_file, OpenFlags}, mm::{copy_from_kernel, translated_refmut, translated_str, MapPermission}, task::{
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    match current_task().unwrap().mmap(start, len, p) {
        Ok(_) => 0,
        Err(_) => -1,
    }
}
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    match current_task().unwrap().unmap(start, len) {
        Ok(_) => 0,
        Err(_) => -1,
    }
}
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(app_inode) = open_file(&translated_str(current_user_token(), path), OpenFlags::RDONLY) {
